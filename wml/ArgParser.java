...
public class ArgParser {
	...
	private final String USAGE_STRING = """
		Usage: Preprocessor [-datadir|-log|-log-t[oken]|-log-p[arse]|-i[nput] filename|-o[utput] filename|-help|-?]
		Options:
			-datadir            Absolute Path to Wesnoth's data directory
			-log                Print all logs (parser and tokenizer)
			-log-parse          Print all parser logs
			-log-token          Print all tokenizer logs
			-warn-parse         Print parser warnings only
			-input filename     Preprocess given file
			-output filename    Write output to given file
