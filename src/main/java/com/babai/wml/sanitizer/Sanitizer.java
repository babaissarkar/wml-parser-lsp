...
public class Sanitizer {
	private static final String[] directives = { "ifdef", "enddef", "define", "undef", "ifhave", "ifver", "arg",
			"endarg", "textdomain" };
	...
	public static void main(String[] args) {
		...
		try {
			if (Files.isDirectory(startPath)) {
				...
				Files.walk(startPath, FileVisitOption.FOLLOW_LINKS).filter(Files::isRegularFile)
		...
		try {
			...
			for (String line : lines) {
				...
				if (trimmed.startsWith("#")) {
					...
					if (hashIndex != -1) {
						if (line.length() == hashIndex + 1 || (line.charAt(hashIndex + 1) != ' '
								&& line.charAt(hashIndex + 1) != '\t' && line.charAt(hashIndex + 1) != '#')) {
