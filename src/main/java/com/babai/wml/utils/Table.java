...
/**
 * Table with Cell-based navigation, automatic indexed lookup, and readable row/column access.
 */
@AIGenerated
public final class Table {
	...
	private Table(Class<?>[] types, String[] names, int... indexedCols) {
		if (types == null || types.length == 0) throw new IllegalArgumentException("must provide at least one column type");
		this.columnTypes = types.clone();
		if (names != null) {
			if (names.length != types.length) throw new IllegalArgumentException("names length must match types length");
			this.columnNames = names.clone();
		} else {
			this.columnNames = new String[types.length];
			for (int i = 0; i < columnNames.length; i++) columnNames[i] = "col" + i;
		}
		for (int col : indexedCols) indices.put(col, new HashMap<>());
	}
	...
	public int columnCount() { return columnTypes.length; }
	...
	public final class Cell<T> {
		...
		public Cell<?> left() { return left; }
		public Cell<?> up() { return up; }
	...
	public final class Row {
		...
		private Row(List<Cell<?>> cells) { this.cells = cells; }
		public Cell<?> getColumn(String colName) { return cells.get(getColumnIndex(colName)); }
		public List<Cell<?>> cells() { return cells; }
		@Override public String toString() { return cells.stream().map(c -> String.valueOf(c.getValue())).toList().toString(); }
	}

	/** Add a row */
	public void addRow(Object... values) {
		if (values.length != columnCount()) throw new IllegalArgumentException("Expected " + columnCount() + " values");
		List<Cell<?>> cellList = new ArrayList<>();
		Row row = new Row(cellList);

		for (int i = 0; i < values.length; i++) {
			Object v = values[i];
			if (v != null && !columnTypes[i].isInstance(v)) throw new ClassCastException("Column " + i + " expects " + columnTypes[i].getName());
			...
			if (idx != null) idx.computeIfAbsent(v, k -> new ArrayList<>()).add(cell);
		}
		rows.add(row);
	}

	/** Get cell by row and column */
	public Cell<?> get(int row, int col) { return rows.get(row).cells.get(col); }

	private int getColumnIndex(String colName) {
		for (int i = 0; i < columnNames.length; i++) if (columnNames[i].equals(colName)) return i;
	...
	public List<Row> getRows(String colName, Object value) {
		...
		if (idx == null) throw new IllegalStateException("Column not indexed: " + colName);
		List<Cell<?>> cells = idx.getOrDefault(value, Collections.emptyList());
		List<Row> result = new ArrayList<>();
		for (Cell<?> c : cells) result.add(c.parentRow);
		return result;
	}
	...
	public String toString() {
		...
		for (int i = 0; i < columnCount(); i++) widths[i] = columnNames[i].length();
		for (Row r : rows) for (int i = 0; i < columnCount(); i++) {
